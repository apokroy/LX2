...
Acknowledgements (for version 4):
 ...
 - Gьnther Schoch for the "RequireDebuggerPresenceForLeakReporting" option.
 - Jan Schlьter for the "ForceMMX" option.
 ...
 - Marcus Mцnnig for the ScanMemoryPoolForCorruptions suggestion and the
...
Change log:
 ...
 - Added the "RequireDebuggerPresenceForLeakReporting" option to only display
   the leak report if the application is run inside the IDE. (Thanks to Gьnther
   Schoch.)
 - Added the "ForceMMX" option, which when disabled will check the CPU for
   MMX compatibility before using MMX. (Thanks to Jan Schlьter.)
 ...
 Version 4.70 (4 August 2006):
  ...
  - Added a global variable "FullDebugModeScanMemoryPoolBeforeEveryOperation".
    ...
    when hunting for elusive memory corruption bugs. (Thanks to Marcus Mцnnig.)
  ...
  - Support added for Free Pascal's OS X and Linux targets, both i386 and
    x86-64. (Thanks to Zoл Peterson - some fixes by Arnaud Bouchez)
